          ...
          <nav aria-label="breadcrumb">
            <ol class="breadcrumb small">
              ...
              <li class="breadcrumb-item">
                ...
                <a :href="$link(`/${routeConst.DAVINCHI_RESOLVE_TEMPLATES}`).url()" v-else-if="isProductType('resolve', submediatype)">
                  <span style="color: #bcc1c3">{{ $t('header.resolve') }}</span>
                </a>
              </li>
              <template v-if="$route.path.match('artists')">
                <li class="breadcrumb-item">
              ...
              <li class="breadcrumb-item" v-else-if="category">
                <a :href="category.url" v-if="category.id">
                  <span style="color: #bcc1c3">{{ category.name }}</span>
                </a>
              </li>
              ...
              <li class="breadcrumb-item" v-if="mediatype && !title">
                <span style="color: #bcc1c3">
                  {{ $term('mediaType', this.mediatype).seo().text() }}
                </span>
              </li>
            </ol>
          </nav>
