...
import * as tools from '@motionelements/helper/tools'
import * as teleport from '@/helpers/teleport.v2'
...
export default {
  ...
  methods: {
    setCurrentMediaType(mediaType) {
      ...
      let info = _.find(mapSupportedMediaTypes, { id: tools.convertRouteMediatypeShort(mediaType) });
      // app no mediatype match fallback
      if (!info) {
        info = _.find(mapSupportedMediaTypes, { id: tools.convertRouteMediatypeShort('video') });
      }
    ...
    fireTeleportForProductPage() {
      ...
      teleport.event('product_viewed', teleportData);
    },
    initProductPage() {
      console.log('INIT PRODUCT PAGE-----');
      const id = '7468870'//this.$route.params.id;

      this.isLoaded = false;

      const isPreviewMode = false//!!this.$route.query.preview;

      if (id) {
        /* this.$store.dispatch('product/getElementDetails', {
            sku: id,
            relationType: 'related_elements',
    ...
    breadcrumbList() {
      ...
      */
        ...
        list.push({
          ...
          // zeus
          nameSeo: 'Morning Light Rays 2 Stock Animation',
