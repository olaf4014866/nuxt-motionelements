// @ts-nocheck
// use store mediatype terms

import get from 'lodash/get';
import isNaN from 'lodash/isNaN';
import snakeCase from 'lodash/snakeCase';

import { listMediatypesSeoName } from '@motionelements/core/src/services/app.service';

let mediatypeList;

async function getMediaTypeList () {
  await listMediatypesSeoName().then((data) => {
    mediatypeList = data.data.data;
  })
};

getMediaTypeList();

console.log('mediatype terms: ', mediatypeList);
...
const getMediaType = function (type) {
  defaultLang.get();
  var mediaType = type;
  ...
  if (!isNaN(Number(type))) {
    const types = mediatypeList.filter(i => i.id === Number(type));
  ...
  function getMtName(code) {
    ...
    const arr = mediatypeList.filter(x => x.code === code);
  ...
  // }
  function getMtObj() {
    const ids = mediatypeList.filter(i => i.code === mediaType);
    return ids[0];
  }
  ...
  var obj = {
    ...
    // code() {
    //   const arr = mediatypeList[defaultLang.get()].filter(x => x.code === mediaType);
    //   if (arr.length) {
    //     ans = arr[0].code;
    //   }
    //   return this;
