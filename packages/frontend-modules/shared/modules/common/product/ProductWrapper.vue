...
export default {
  ...
  computed: {
    ...
    isExpanded() {
      ...
      // those mediatype enable on mobile
      ...
      status = this.options.hoverEvent;

      if (enableList.indexOf(this.element.media_type) >= 0) {
        forceExpanded = true;
      }

      // force enable when is collection on mobile
      if (this.element.object === 'collection') {
        forceExpanded = true;
      }
      ...
      if (!isMobile && ['3d-model', '3d_model'].indexOf(this.element.media_type >= 0)) {
        status = true;
      }

      // console.log('isExpanded - ', isMobile, this.isImage, this.cardHoverStatus);
    ...
    isImage() {
      ...
      if (imageList.indexOf(this.element.media_type) >= 0) {
        status = true;
      }
    ...
    isAudio() {
      ...
      if (audioList.indexOf(this.element.media_type) >= 0) {
        status = true;
      }
    ...
    isLargeCard() {
      ...
      if (largeCardType.indexOf(this.element.media_type) >= 0) {
        status = true;
      }
  ...
  methods: {
    clickCheckboxBar() {
      ...
      const isAdd = (selectedElementsId.indexOf(clickElementId) >= 0) ? true : false;
