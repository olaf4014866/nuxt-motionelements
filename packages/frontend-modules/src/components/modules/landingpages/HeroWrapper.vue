<template>
  ...
  >
    ...
    <transition name="fade">
      <FileDropHelp
        v-if="showAiSearchHelp"
        :class="textStyles.invertColor ? 'component-dark' : ''" />
    </transition>
    ...
    <FileDropUpload
      v-if="isFileDrop && !$route.name.match('home')"
      :options="{ fullscreen: false }"
      :isFileDropClick="showAiSearchUpload"
    ></FileDropUpload>
...
export default {
  name: 'HeroWrapper',
  components: {
    HeroTitle: () => import('@/components/modules/landingpages/hero/HeroTitle.vue'),
    FileDropHelp: () => import('@/components/modules/common/FileDropHelp.vue'),
    FileDropUpload: () => import('@/components/modules/common/FileDropUpload.vue'),
  },
  props: {
    isFileDrop: {
      type: Boolean,
  ...
  data() {
    return {
      showAiSearchHelp: false,
      showAiSearchUpload: false,
    };
  },
  computed: {
    getTextClasses() {
  ...
  mounted() {
    this.$root.$on('toggle-search-hint', (payload) => {
      this.toggleSearchHint(payload);
    });
  },
  methods: {
    toggleSearchHint(payload) {
      if (payload.type === 'help') {
        this.showAiSearchHelp = payload.value;
      } else if (payload.type === 'upload') {
        this.showAiSearchUpload = payload.value;
      }
    },
    getBgImgUrl(url) {
