...
import { initAccount, getUiModeByCookie } from '@motionelements/core/src/services/account.service';
...
//zeus
const isPrerender = true//process.env.VUE_APP_PRERENDER === 'true' || (navigator.userAgent || '').indexOf('HeadlessChrome') !== -1;
...
export function detectBrowser() {
  ...
  console.log('userAgent.isMobile', isMobile);
  store.dispatch('site/setIsMobile', isMobile);
}
...
function initUi() {
  const mode = getUiModeByCookie();

  console.log('init UI MODE', mode);
...
export async function bootstrapApp() {
  ...
  // set ui mode
  const promise = initAccount();

  if (promise instanceof Promise) {
    promise.then((response) => {
      // init currencies after member language is set
      console.log('bootstrap1', response);
      initCurrencies();
    });
  } else {
